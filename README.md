# Framework for Running IP over BLE Extended Advertisement Experiments

## About
This repository contains all the artifacts necessary to reproduce the results of our paper **'IPv6 over Bluetooth Advertisements: An alternative approach to IP over BLE'**.

## Repository Structure
The following list gives a high-level rundown on the purpose of each folder in this repository:

- `ea`: this folder contains all the experiment descriptions, as well as the corresponding RIOT applications
...
All raw experiment data used in our paper is available via our Nextcloud instance at https://box.fu-berlin.de/s/2JJNWz5fDZ5jpyc/download/ccr_ipbleadv_logs.zip

**Note:** For the final version of the paper all experiment data will be uploaded to zenodo (https://zenodo.org) to make them accessible for at least 10 years.

To make this data accessible to our experiment framework and tooling located in this repository, we included a script that will automatically download the raw data from our Nextcloud into the `results/logs`. Simply run the
```bash
...
**Note:** the download my take a while as the script is downloading roughly 500Mb of data and the extracted files will take roughly 3Gb of local storage.
...
| :--         | :-- |
| Fig. 2      | `tools/fig_perfoverview.py` |
| Fig. 3      | `tools/fig_retransmission.py` |
| Fig. 4 (a)  | `tools/fig_24h_pdr.py` |
| Fig. 4 (b)  | `tools/fig_24h_bgtraffic.py` |
| Fig. 5 (a)  | `tools/fig_phyusage_tx.py` |
| Fig. 5 (b)  | `tools/fig_phyusage_rx.py` |
| Fig. 6 (a)  | `tools/fig_ram.py` |
| Fig. 6 (b)  | `tools/fig_rom.py` |
| Fig. 7 (a)  | `tools/fig_mem_1s.py` |
| Fig. 7 (b)  | `tools/fig_mem_5s.py` |
...
```bash
git clone --branch ipbleadv https://github.com/haukepetersen/RIOT lib/RIOT
git clone --branch ipbleadv https://github.com/haukepetersen/mynewt-nimble lib/NIMBLE
```
