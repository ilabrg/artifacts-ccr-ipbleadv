...
class LLStats:
    ...
    def __init__(self, ana):
        ...
        self.jelling_all = []
        self.jelling_pn  = {n: [] for n in self.ana.desc["used_nodes"]}

        self.re_advstats = re.compile(r'^>? *ll,'
                                      r'(?P<tx_c0>\d+),(?P<tx_c1>\d+),(?P<tx_c2>\d+),(?P<tx_c3>\d+),'
                                      r'(?P<rx_c0>\d+),(?P<rx_c1>\d+),(?P<rx_c2>\d+),(?P<rx_c3>\d+),'
                                      r'(?P<jel_dup>\d+),(?P<jel_known>\d+),(?P<jel_unknown>\d+),'
                                      r'(?P<rx_er_c0>\d+),(?P<rx_er_c1>\d+),(?P<rx_er_c2>\d+),(?P<rx_er_c3>\d+),'
                                      r'(?P<aux_ns_c0>\d+),(?P<aux_ns_c1>\d+),(?P<aux_ns_c2>\d+),(?P<aux_ns_c3>\d+),'
                                      r'(?P<ae_drop_c0>\d+),(?P<ae_drop_c1>\d+),(?P<ae_drop_c2>\d+),(?P<ae_drop_c3>\d+),'
                                      r'(?P<crc_err_c0>\d+),(?P<crc_err_c1>\d+),(?P<crc_err_c2>\d+),(?P<crc_err_c3>\d+),'
                                      r'(?P<aux_chain_err>\d+),(?P<jel_broken_data>\d+),'
                                      r'(?P<rssi_c0>-?\d+),(?P<rssi_c1>-?\d+),(?P<rssi_c2>-?\d+),(?P<rssi_c3>-?\d+),'
                                      r'(?P<rssi_crc_c0>-?\d+),(?P<rssi_crc_c1>-?\d+),(?P<rssi_crc_c2>-?\d+),(?P<rssi_crc_c3>-?\d+)')
        self.re_txstats  = re.compile(r'^>? *ll(?P<conn>\d+),(?P<stats>[0-9a-zA-Z]{80})')
        self.re_phystats = re.compile(r'^>? *ls,(?P<dur>\d+),(?P<free>\d+),'
                                      r'(?P<rx_cnt>\d+),(?P<rx_cnt_off>\d+),(?P<rx>\d+),'
                                      r'(?P<tx_cnt>\d+),(?P<tx_cnt_off>\d+),(?P<tx>\d+)')
    ...
    def update(self, time, node, line):
        m = self.re_advstats.search(line)
        if m:
            # normalize RSSI values
            cnt_rx = [int(m.group(f'rx_c{i}')) for i in range(4)]
            cnt_crc_er = [int(m.group(f'crc_err_c{i}')) for i in range(4)]
            rssi_all = [int(m.group(f'rssi_c{i}')) for i in range(4)]
            rssi_crc_er = [int(m.group(f'rssi_crc_c{i}')) for i in range(4)]
            rssi_norm_crcok = [0] * 4
            rssi_norm_crcnok = [0] * 4
            for i in range(4):
                if (cnt_rx[i] - cnt_crc_er[i]) > 0:
                    rssi_norm_crcok[i] = rssi_all[i] / (cnt_rx[i] - cnt_crc_er[i])
                if cnt_crc_er[i] > 0:
                    rssi_norm_crcnok[i] = rssi_crc_er[i] / cnt_crc_er[i]

            adv_evt = {
                "time": time,
                "node": node,
                "tx": [int(m.group(f'tx_c{i}')) for i in range(4)],
                "rx": cnt_rx,
                "jel_dup": int(m.group("jel_dup")),
                "jel_known": int(m.group("jel_known")),
                "jel_unkown": int(m.group("jel_unknown")),
                "jel_broken": int(m.group("jel_broken_data")),
                "rx_err": [int(m.group(f'rx_er_c{i}')) for i in range(4)],
                "aux_not_sched": [int(m.group(f'aux_ns_c{i}')) for i in range(4)],
                "adv_evt_drop": [int(m.group(f'ae_drop_c{i}')) for i in range(4)],
                "crc_err": cnt_crc_er,
                "aux_chain_err": int(m.group("aux_chain_err")),
                "rssi_raw_ok": rssi_all,
                "rssi_raw_crcer": rssi_crc_er,
                "rssi_norm_crc_ok": rssi_norm_crcok,
                "rssi_norm_crc_nok": rssi_norm_crcnok,
            }
            self.jelling_all.append(adv_evt)
            self.jelling_pn[node].append(adv_evt)
        ...
        m = self.re_phystats.search(line)
        if m:
            ...
            phy_evt = {
                ...
                "rx_cnt_off": int(m.group("rx_cnt_off")),
                "rx_tim": int(m.group("rx")),
                "tx_cnt": int(m.group("tx_cnt")),
                "tx_cnt_off": int(m.group("tx_cnt_off")),
    ...
    def summary(self):
        ...
        if len(self.jelling_all) > 0:
            sums = {n: {"tx": 0, "rx": 0, "crc_err": 0, "rx_err": 0, "aux_not_sched": 0, "adv_evt_drop": 0, "jel_known": 0, "jel_unkown": 0, "jel_dup": 0, "jel_broken": 0} for n in (list(self.jelling_pn.keys()) + ["sum"])}
            for evt in self.jelling_all:
                for s in ("tx", "rx", "crc_err", "rx_err", "aux_not_sched", "adv_evt_drop"):
                    sums[evt["node"]][s] += sum(evt[s])
                    sums["sum"][s] += sum(evt[s])
                for s in ("jel_known", "jel_unkown", "jel_dup", "jel_broken"):
                    sums[evt["node"]][s] += evt[s]
                    sums["sum"][s] += evt[s]

            self.ana.statwrite("JELLLING link layer stats")
            self.ana.statwrite(f'{"NODE":>15} {"TX":>9} {"RX":>9} {"CRC_ER":>9} {"RX_ER":>9} {"AUX_NOSCH":>9} {"AEVT_DROP":>9} {"OK":>9} {"NOK":>9} {"DUP":>9} {"BROKEN":>9}')
            for n in sums:
                self.ana.statwrite(f'{n:>15}', end=False)
                for s in ("tx", "rx", "crc_err", "rx_err", "aux_not_sched", "adv_evt_drop", "jel_known", "jel_unkown", "jel_dup", "jel_broken"):
                    self.ana.statwrite(f' {sums[n][s]:>9}', end=False)
                self.ana.statwrite("")
            self.ana.statwrite("\n")
    ...
    def plot_chanrate(self, all_chan=False, binsize=30, nodes=None, excl_nodes=None, timespan=None):
        if len(self.events) == 0:
            print(f'llstats: SKIPPING chanrate')
            return
    ...
    def plot_rateline(self, nodes=None, binsize=5.0, timespan=None, ylim=None):
        if len(self.events) == 0:
            print("llstats: SKIPPING rateline")
            return
    ...
    def plot_rateline_pn(self, nodes=None, binsize=5.0, timespan=None, ylim=None):
        if len(self.events) == 0:
            print("llstats: SKIPPING rateline_pn")
            return
    ...
    def plot_bufusage(self, nodes=None, timespan=[None, None], xlim=None):
        ...
        for n in self.buf:
            if len(self.buf[n]) == 0:
                print("llstats: SKIPPING bufusage")
                return
        ...
        for n in self.ana.nsort(self.buf):
            ...
            data.append({
                "x": [self.ana.t_norm(e["time"]) for e in self.buf[n]],
    ...
    def plot_phy_usage(self, stat="rx", nodes=None, timespan=[None, None], xlim=None):
        ...
        if len(self.phy_all) == 0:
            print(f'llstats: SKIPPING plot_phy_usage ({stat})')
            return
    ...
    def plot_phy_usage_sum(self, stat=["rx", "tx", "aggr"], nodes=None, binsize=10.0, timespan=[None, None]):
        cfg = self.ana.plotsetup(nodes, binsize, timespan)

        if len(self.phy_all) == 0:
            print("llstats: SKIPPING phy_usage_sum")
            return
        ...
        title = f'Average PHY Utilization (Binsize {cfg["binsize"]}s)'
        if nodes != None:
            suffix += "_" + "-".join(nodes)
            title += " [" + "-".join(nodes) + "]"
    ...
    def plot_phy_cnt(self, stat="tx", nodes=None, timespan=[None, None], xlim=None):
        ...
        if len(self.phy_all) == 0:
            print(f'llstats: SKIPPING phy_cnt ({stat})')
            return
    ...
    def plot_phy_cnt_sum(self, stat=["rx", "tx", "aggr"], nodes=None, binsize=15.0, timespan=[None, None]):
        ...
        if len(self.phy_all) == 0:
            print("llstats: SKIPPING phy_cnt_sum")
            return
    ...
    def plot_jelling_pkt_ratio(self, nodes=None, binsize=10.0, timespan=[None, None]):
        if len(self.jelling_all) == 0:
            print("llstats: SKIPPING jelling_pkt_ratio")
            return
        ...
        for evt in self.jelling_all:
            ...
            if evt["time"] >= curbin + cfg["binsize"]:
                data["x"].append(self.ana.t_norm(curbin))
                if cnt_kown > 0:
                    data["y"].append(cnt_unkown / cnt_kown)
                else:
                    data["y"].append(0.0)
                cnt_kown = 0
                cnt_unkown = 0
                curbin += cfg["binsize"]
            cnt_kown += evt["jel_known"]
            cnt_unkown += evt["jel_unkown"]

        xt = self.ana.plotter.get_ticks([data["x"]], None)
        suffix = "jel_pkt_ratio"
        title = f'Ratio of non-related packets to BLE-ADV packets (binsize: {cfg["binsize"]}s)'
        if nodes != None:
            suffix += "_" + "-".join(nodes)
            title += " [" + "-".join(nodes) + "]"
    ...
    def plot_jelling_adv_pkt_cnt(self, nodes=None, binsize=10.0, timespan=[None, None]):
        if len(self.jelling_all) == 0:
            print("llstats: SKIPPING plot_jelling_nonrel_pkt_cnt")
            return

        cfg = self.ana.plotsetup(nodes, binsize, timespan)

        data = [
            {"x": [], "y": [], "label": "Related packets"},
            {"x": [], "y": [], "label": "Unrelated packets"},
        ]
        ...
        for evt in self.jelling_all:
            ...
            if evt["time"] >= curbin + cfg["binsize"]:
                for i in range(2):
                    data[i]["x"].append(self.ana.t_norm(curbin))
                    data[i]["y"].append(pkt_cnt[i])
                    pkt_cnt[i] = 0
                curbin += cfg["binsize"]
            pkt_cnt[0] += evt["jel_known"]
            pkt_cnt[1] += evt["jel_unkown"]

        xt = self.ana.plotter.get_ticks([data[0]["x"]], None)
        suffix = "jel_adv_pkt_cnt"
        title = f'Number of received advertising packets (binsize: {cfg["binsize"]}s)'
        if nodes != None:
            suffix += "_" + "-".join(nodes)
            title += " [" + "-".join(nodes) + "]"
